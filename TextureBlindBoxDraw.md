# Skill: 纹理顺势盲盒创意绘画 TextureBlindBoxDraw_v1.2
...
| 字段 | 值 |
|------|----|
| skill_id | TextureBlindBoxDraw_v1.2 |
| name | 纹理顺势盲盒创意绘画 |
| version | 1.2 |
| type | 图像局部增量增强，非全图生成 |
...
- visibility_level: 可见度档位 1‑5，默认3；用户反馈太小/不明显时使用4
...
## 可见度规则 visibility_level

可见度独立于涂鸦占比。用户反馈“太小”“不明显”“看不出来”时，优先提高可见度，而不是盲目增加全图涂鸦面积。

|档位|说明|
|---|---|
|1 极隐约|只在用户明确要求很淡时使用，全尺寸仍需看见|
|2 轻微|小而清楚，适合干净浅色背景|
|3 默认|主事件清楚可见，1024px图中主涂鸦最长边约90‑160px，512px预览仍能读出动作|
|4 增强|用户说太小/不明显/看不出来时使用，主涂鸦最长边约150‑230px，可加动作线和道具增强可读性|
|5 大胆|需要用户确认；更醒目但仍局部，不超过整体30%涂鸦面积|

硬性检查：512px缩略图里必须能看出主事件；如果只剩几个小眼睛、小笑脸，视为失败。复杂水面、黑盘、阴影、反光背景上，允许给黑线加极窄浅色留边/高光间隔，避免线条丢进纹理里。
...
- suggest_user_msg：
> 已解析图片，自动启用【纹理顺势盲盒创意绘画】，选用【物体本体拟人】，强度档位2、可见度3。会完整保留饭菜、餐盘、筷子、汤水，只在真实食物轮廓之间增加一名能看清的测绘员、一面小旗和少量路线细节。可以告诉我调强、调淡、换主题，或者指定一个隐藏事件。
